...
from tf2_ros import Buffer, TransformListener #tf2_ros package

from jsonschema import Draft202012Validator

from cleaner_system1.core.plan_schema import HIGH_LEVEL_PLAN_SCHEMA
from cleaner_system1.core.guards import eval_guard
from cleaner_system1.core.replan import emit_replan
from cleaner_system1.core.vision_context import VisionCache
from cleaner_system1.core.state_publisher import make_state
from cleaner_system1.io.topics import create_publishers, create_subscriptions
from cleaner_system1.utils.geometry import quat_to_yaw
...
class System1ExecutorNode(Node): # rclcp.node.Node 상속
    """
    ...
    def __init__(self):
        ...
        self.current_index = -1 #실행 중인 스탭. 복수의 플랜x를 받아와 순차적으로 실행
        ...
        큐 상태. 
         ...
         - done: 플랜의 모든 스텝 완료
        """
        self.mission_id = "" #수행중인 미션 추적 -> 어떤 미션의 어떤 step에서 실패했는지 추적
        ...
        # ---------- Pub/Sub ----------
        pubs = create_publishers(self) #별도 IO관리 모듈(cleaner_system1.io.topics)로 분리하여 정의 > 토픽 설정에 종속x
    ...
    def _do_track(self, params: dict, plan: Optional[dict]) -> bool:
        ...
        succ_wait = float(
            params.get(
                "succ_wait_sec",
                rules.get("track_initial_success_wait_sec", 6.0),
            )
        )
    ...
    def publish_state(self): # 다음 항목을  하나의 상태 메시지로 통합해 발송
        ...
        self.pub_cleaner.publish(msg) #publish 상태 메시지
