...
def _lookup_robot_yaw(
    tf_buffer: Buffer, global_frame: str, base_frame: str, node: Node
) -> Tuple[bool, float]:
    try:
        tf = tf_buffer.lookup_transform(global_frame, base_frame, rclpy.time.Time())
        return True, float(_quat_to_yaw(tf.transform.rotation))
    except Exception as e:
        node.get_logger().warn(
            f"[scan] robot yaw TF lookup failed ({global_frame}->{base_frame}): {e}"
        )
        return False, 0.0
...
def exec_scan(
    ...
    *,
    ...
    # 유틸
    def publish_scan_motion(yaw_rate: float):
        tw = Twist()
    ...
    try:
        ...
        while rclpy.ok():
            ...
            publish_scan_motion(yaw_rate_cmd)
        ...
        pitch_dir  = +1.0
        yaw_ref_ok, yaw_ref = _lookup_robot_yaw(
            tf_buffer, tf_global_frame, tf_base_frame, node
        )
        if not yaw_ref_ok:
            stop_all()
            return False
        ...
        while rclpy.ok():
            ...
            yaw_ok, robot_yaw = _lookup_robot_yaw(
                tf_buffer, tf_global_frame, tf_base_frame, node
            )
            if not yaw_ok:
                stop_all(); time.sleep(dt); continue
            yaw_rel = _wrap_to_pi(robot_yaw - yaw_ref)
