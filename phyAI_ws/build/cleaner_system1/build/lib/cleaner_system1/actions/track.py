...
class Tracker:
    """
    목적:
      - 카메라는 '픽셀 오차'로 대상 락(cleaner yaw/pitch)
      - 바디는 TF yaw_rel PD정렬
      - Depth ROI로 거리 유지
    ...
    def __init__(
        ...
        self.has_gimbal = gimbal_pub is not None
    ...
    def _log_status_block(
        ...
        lines = [
            ...
            f"- Command   : wz={_fmt(wz,'{:+.3f}')}, vx={_fmt(vx,'{:+.3f}')}  |  yaw_ctrl={_fmt(yaw_rate,'{:+.4f}')}",
        ]
    ...
    def _log_success_window(self, in_yaw: bool, in_dist: bool,
        ...
        lines = [
            "[SUCCESS WINDOW]",
            f"- Yaw     : {ok(in_yaw)}  (hold {yaw_hold:.2f}/{success_hold_sec:.2f}s, thr ≤{success_yaw_deg:.1f}°)",
    ...
    def _control_step(self, cfg: Dict[str, Any]) -> Tuple[bool, bool, float, float, float, float, Optional[float], Optional[float]]:
        ...
        yaw_rate_cap = float(cfg.get("yaw_rate_cap", cfg.get("gimbal_rate_cap", 1.2)))
        ...
        yaw_rate_sat = _clamp(yaw_rate_cmd, -yaw_rate_cap, yaw_rate_cap)
        if abs(yaw_rate_cmd) < yaw_rate_cap * 0.999:
        ...
        pitch_rate = 0.0
        yaw_err_rad = self._px_to_yaw_rad(err_px_x, fw, cam_hfov_deg)
        yaw_err_deg = abs(math.degrees(yaw_err_rad))
        aligned = yaw_err_deg <= float(cfg.get("yaw_deadband_enter_deg", 4.0))
        wz = _clamp(yaw_rate, -float(cfg.get("wz_cap", 3.0)), +float(cfg.get("wz_cap", 3.0)))
        if aligned and abs(wz) < float(cfg.get("wz_stop_eps", 0.5)):
            wz = 0.0
