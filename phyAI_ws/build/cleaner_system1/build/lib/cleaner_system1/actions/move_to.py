...
def _flush_zero_cmd(node: Node, times: int = 6, sleep_s: float = 0.05) -> None:
    """Flush zero cmd_vel a few times to prevent residual rotation at step transitions."""
    zero = Twist()
    zero.linear.x = 0.0
    zero.angular.z = 0.0
    pub = getattr(node, "pub_cmd", None)
    if pub is None:
        return
...
class Nav2Navigator:
    ...
    def __init__(
        ...
        frame_id: str = "map",
        server_wait_timeout_sec: float = 20.0,
    ):
        self._node = node
    ...
    def start(self, goal: Dict[str, Any]):
        ...
        while not self._client.wait_for_server(timeout_sec=0.2):
            if not rclpy.ok():
                ...
                return None, None
            if time.time() - t0 > self._server_wait_timeout_sec:
                self._node.get_logger().error(
                    f"[Nav2Navigator] action server '{self._action_name}' 연결 실패 "
                    f"(waited {self._server_wait_timeout_sec:.1f}s)"
                )
                return None, None
