...
from cleaner_msgs.msg import CleanerState, PlanCommand
...
class System2Node(Node):
    def __init__(self):
        ...
        self.state_sub = self.create_subscription(
            CleanerState, '/cleaner_state', self.state_callback, 10
        )
    ...
    # ----------- /cleaner_state 콜백 -----------
    def state_callback(self, msg: CleanerState):
        try:
            plan = {}
            try:
                plan = json.loads(msg.plan_json) if msg.plan_json else {}
            except Exception:
                plan = {}

            mission_id = plan.get("mission_id")
            current_task = msg.system_state or msg.queue_status

            vision_summary = None
            try:
                vision = json.loads(msg.vision_json) if msg.vision_json else {}
                objects = vision.get("objects") or vision.get("targets") or []
                if objects:
                    first = objects[0]
                    vision_summary = f"{len(objects)} objects, main={first.get('class', 'object')}"
                else:
                    vision_summary = "no visible targets"
            except Exception:
                vision_summary = None

            self.latest_state = System1State(
                mission_id=mission_id,
                system1_state=msg.system_state,
                current_task=current_task,
                step_index=msg.current_index,
                pose={
                    "x": msg.pose_x,
                    "y": msg.pose_y,
                    "yaw_deg": msg.pose_yaw * 180.0 / 3.141592653589793,
                },
                vision_snapshot={"summary": vision_summary} if vision_summary else None,
                notes=f"battery_soc={msg.battery_soc:.2f}, max_speed={msg.max_speed:.2f}",
            )
        except Exception as e:
            self.get_logger().warn(f"[System2] cleaner_state parse failed: {e}")
